from src.GameEngine.Render.py_aseprite import AsepriteFile
from src.GameEngine.Render.py_aseprite import CelChunk
import pygame
import time
...
class Animation(object):
    ...
    def __init__(self, _filedir):
        ...
        # load aseprite file
        self.aseprite_file = self.parse_file(_filedir)
        ...
        self.size = self.get_animation_sizes()

    def get_animation_sizes(self):
        self.size = [frame.get_rect() for frame in self.animation_frames]
        return self.size
        ...
        for y in range(cel.data['height']):
            for x in range(cel.data['width']):
                ...
                if (channelred + channelgreen + channelblue == 0):
                    ...
                    frame.set_at((x + cel.x_pos, y + cel.y_pos),
                                 pygame.Color(channelred, channelgreen, channelblue, channelalpha))
...
class AnimationManager(object):
    ...
    def __init__(self, _animation_list, _draw_surface: pygame.Surface):
        """Init
    ...
    def update_self(self, frame_x, frame_y, flip_x=False, flip_y=False, angle=0, scale=1):
        ...
        # Blit frame
        transformed_surface = pygame.transform.rotate(
            pygame.transform.flip(self.current_animation.animation_frames[self.framenum], flip_x, flip_y), angle)
        transformed_surface = pygame.transform.scale(transformed_surface, (transformed_surface.get_width() * scale,
                                                                           transformed_surface.get_height() * scale))
        self.draw_surface.blit(transformed_surface, (frame_x, frame_y))
